...
public class ItemForciciumCell extends ItemMFFSBase {
    private boolean active = false;

    public ItemForciciumCell() {
                            ...
                            if (!slot.getStack().isEmpty()) {
                                if (slot.getStack().getItem() == ModItems.FORCICIUM) {
                                    ...
                                    if (slot.getStack().getCount() > 1) {
                                        ItemStack forcicium = new ItemStack(ModItems.FORCICIUM, slot.getStack().getCount() - 1);
                                        slot.putStack(forcicium);
                                    } else {
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
        String tool = String.format("%d / %d  Forcicium", getForceciumlevel(stack), getMaxForceciumlevel());
        tooltip.add(tool);
    }
    ...
    public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer playerIn, EnumHand handIn) {
        if (!world.isRemote) {
            if (!active) {
                active = true;
                playerIn.sendMessage(new TextComponentTranslation("itemInfo" + ".forciciumCellActive"));
            } else {
                active = false;
                playerIn.sendMessage(new TextComponentTranslation("itemInfo" + ".forciciumCellInactive"));
            }
        }
